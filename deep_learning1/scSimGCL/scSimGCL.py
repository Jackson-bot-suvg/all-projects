...
class GraphConstructor(nn.Module):
    """
    注意：graph_head=1 时，这里不会真正分多头。
    phi=0.0 => adj 几乎全连接。
    """
    def __init__(self, input_dim, h, phi, dropout=0):
        super(GraphConstructor, self).__init__()
        assert input_dim % h == 0, "input_dim 必须能被 h 整除"
    ...
    def forward(self, query, key):
        query, key = [
            l(x).view(query.size(0), -1, self.h, self.d_k).transpose(1, 2)
            for l, x in zip(self.linears, (query, key))
        ]
        attns = self.attention(query.squeeze(2), key.squeeze(2))
        adj = torch.where(
            attns >= self.phi,
            torch.ones(attns.shape).to(device),
            torch.zeros(attns.shape).to(device)
        )
        return adj
...
def DataAug(x, adj, prob_feature, prob_edge):
    """
    由于 config 里设了 prob_feature=0.0, prob_edge=0.0，
    这里基本不会做任何丢弃。
    """
...
def cl_loss(z, z_aug, adj, tau, hidden_norm=True):
    """
    你可以在此处加 debug. 不过现在 config 里 lambda_cl=0.0 => 本函数不会实际参与loss
    """
    eps = 1e-8
...
def final_cl_loss(alpha1, alpha2, z, z_aug, adj, adj_aug, tau, hidden_norm=True):
    # 因为 lambda_cl=0.0，不会真正加进 total_loss.
    return (
        alpha1 * cl_loss(z, z_aug, adj, tau, hidden_norm)
      + alpha2 * cl_loss(z_aug, z, adj_aug, tau, hidden_norm)
    )


class Model(nn.Module):
    """
    包含 GraphConstructor, GCNConv, w_imp, MLP(对比)。
    但对比损失实际上 lambda_cl=0 => 不会影响 total_loss。
    """
    def __init__(self,
                 input_dim, graph_head, phi,
                 gcn_dim, mlp_dim,
                 prob_feature, prob_edge,
                 tau, alpha, beta,
                 dropout):
    ...
    def forward(self, x):
        ...
        loss_cl = final_cl_loss(
            self.alpha, self.beta,
            z_mlp, z_mlp_aug,
            adj, adj_aug,
            self.tau, hidden_norm=True
        )
